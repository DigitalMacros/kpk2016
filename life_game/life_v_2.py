...
def step_back():
    ...
    if begin:
        start_or_stop()
    if len(steps_list) > 0:
        field.matrix  = [[0] * field.cells_x_count for i in range(field.cells_y_count)]
        for y, x in steps_list[-1]:
            field.matrix[y][x] = 1
        field.all_paint()
        steps_list.pop(-1)
    else:
        print('Нет данных о предыдущих поколениях')
...
def step_forward():
    ...
    if begin:
        start_or_stop()
    for y in range(field.cells_y_count):
        for x in range(field.cells_x_count):
            if field.matrix[y][x] == 1:
                coord_list.append([y, x])
    steps_list.append(coord_list)
...
def init_window():
    ...
    button_step_back = Button(root, text='<--', width=4, command=step_back)
    button_step_back.place(x = canvas_width+10,y = 280)
    button_step_forward = Button(root, text='-->', width=4, command=step_forward)
    button_step_forward.place(x = canvas_width+58,y = 280)
